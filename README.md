...
- workflow runs on push and manual dispatch, while concurrency keeps only one active run
...
- `scripts/build_kernel_bundle.sh` now auto-discovers a kernel from `/lib/modules/<release>/vmlinuz`, `/usr/lib/modules/<release>/vmlinuz`, `/boot`, and fallback `vmlinuz-*` paths while skipping unreadable candidates (or uses `KERNEL_SRC=...` when set).
